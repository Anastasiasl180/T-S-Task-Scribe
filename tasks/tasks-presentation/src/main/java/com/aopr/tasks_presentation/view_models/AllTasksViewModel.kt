...
@KoinViewModel
class AllTasksViewModel(private val tasksUseCase: TasksUseCase) :
    ViewModelKit<AllTasksEvents, AllTasksUiEvents>() {
    ...
    override fun onEvent(event: AllTasksEvents) {
        when (event) {
            ...
            is AllTasksEvents.AddCategoryForDeletion -> {
                _selectedTasksToDelete.add(event.task)
            }

            AllTasksEvents.DeleteSeveralCategories -> {
            ...
            is AllTasksEvents.RemoveCategoryForDeletion -> {
                _selectedTasksToDelete.remove(event.task)
            }
        }
